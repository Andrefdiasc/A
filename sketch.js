...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    //scoring
    if (frameCount % 1 === 0) {
      score = score + Math.round(frameCount/2000 + 1);
    }
    ...
    if(touches.length > 0 || keyDown("space")&& trex.y >= windowHeight/2) {
        trex.velocityY = -12;
        jumpSound.play();
        touches = [];
    }
...
function spawnObstacles(){
 if (frameCount % 60 === 0){
   var obstacle = createSprite(windowWidth,windowHeight/2 - 15,10,40);
...
function spawnClouds() {
  ...
  if (frameCount % 60 === 0) {
    ...
    cloud.y = Math.round(random(10,windowHeight-100));
